...
// Lista de dias para o Checkbox
                  ...
                  trailing: Row(
                    ...
                    children: [
                      IconButton(
                        ...
                        onPressed: () => _abrirFormulario(item), // Passa o objeto Turma
                      ),
                      IconButton(
  ...
  void _abrirFormulario(Turma item) {
    ...
    if (item != null) {
      ...
      _diasSelecionados = List.from(item.diasSemana as Iterable<dynamic>); // Copia a lista para evitar bugs
      ...
      if (item.horaFim.isNotEmpty) {
        ...
        _horaFim = TimeOfDay(hour: int.parse(partes[0]), minute: int.parse(partes[1]));
      }
    } else {
    ...
    _diasSelecionados = item.diasSemana ?? [];
                ...
                return Padding(
                  padding: EdgeInsets.only(
                    bottom: MediaQuery
                        .of(context)
                        .viewInsets
                        .bottom, // Ajusta o espaço do teclado
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<int>(
                          value: _idBolsistaSelecionado,
                          decoration: const InputDecoration(
                            labelText: "Selecionar Bolsista",
                            prefixIcon: Icon(Icons.school),
                            border: OutlineInputBorder(),
                          ),
                          items: _apenasBolsistas.map((b) =>
                              DropdownMenuItem(
                                value: b.id,
                                child: Text(b.nome),
                              )).toList(),
                          ...
                          children: [
                              ...
                              child: OutlinedButton.icon(
                                ...
                                onPressed: () async {
                                  final time = await showTimePicker(
                                    ...
                                    helpText: "SELECIONE O HORÁRIO DE INÍCIO", // Texto no topo
                                    confirmText: "DEFINIR",
                                    cancelText: "VOLTAR",
                                    // TROQUE O MODO AQUI:
                                    initialEntryMode: TimePickerEntryMode.input, // Muda para entrada de teclado
                                    builder: (context, child) {
                                      return Theme(
                                        data: Theme.of(context).copyWith(
                                          colorScheme: const ColorScheme.light(
                                            primary: Colors.teal, // Cor dos ponteiros/botão
                                  ...
                                  if (time != null) setModalState(() => _horaInicio = time);
                                },
                              ),
                            ),
                            ...
                            Expanded(
                              child: OutlinedButton.icon(
                                ...
                                label: Text(
                                    _horaFim == null ? "Fim" : _horaFim!.format(
                                        context)),
                                onPressed: () async {
                                  final time = await showTimePicker(
                                    context: context,
                                    initialTime: _horaFim ?? TimeOfDay.now(),
                                    helpText: "SELECIONE O HORÁRIO DE FIM", // Texto no topo
                                    confirmText: "DEFINIR",
                                    cancelText: "VOLTAR",
                                    // TROQUE O MODO AQUI:
                                    initialEntryMode: TimePickerEntryMode.input, // Muda para entrada de teclado
                                  ...
                                  if (time != null) setModalState(() => _horaFim = time);
                                },
                              ),
                            ),
                          ],
                        ...
                        const Text("Dias da Semana:", style: TextStyle(
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            if (_formKeyModal.currentState!
                                .validate()) {

                              if (_horaInicio == null || _horaFim == null || _idBolsistaSelecionado == null) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text("Preencha horários e selecione um bolsista!")),
                                );
                                return;
                              }

                              Usuario bolsistaEscolhido = _apenasBolsistas.firstWhere(
                                    (b) => b.id == _idBolsistaSelecionado,
                              );

                              String formatTime(TimeOfDay t) =>
                                  "${t.hour.toString().padLeft(2, '0')}:${t
                                      .minute
                                      .toString()
                                      .padLeft(2, '0')}:00";
