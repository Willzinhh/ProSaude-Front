...
class _AvaliacaoFormScreenState extends State<AvaliacaoFormScreen> {
  ...
  // --- WIDGETS AUXILIARES PARA PRESERVAR CÓDIGO LIMPO ---

  // Helper para Inputs de Texto Padrão
  Widget _buildTextField({required String label, required Function(String) onChanged, bool isNumber = false}) {
  ...
  Widget _buildPosturaDropdown({required String label, required String? valorAtual, required List<String> opcoes, required Function(String?) onChanged, required List<String> opces}) {
    return Padding(
      ...
      child: Column(
        children: [
          GroupContainer(
            ...
            children: [
              _buildTextField(label: 'Profissão', onChanged: (val) => _avaliacao.anaProfi = val),
              const SizedBox(height: 12),
              _buildTextField(label: 'Horas de Trabalho Diárias', isNumber: true, onChanged: (val) => _avaliacao.anaHsTrab = double.tryParse(val)),
              const SizedBox(height: 12),
              _buildTextField(label: 'Turno de Trabalho', onChanged: (val) => _avaliacao.anaTurnTrab = val),
              const SizedBox(height: 12),
              SwitchListTile(
              ...
              if (_avaliacao.anaFuma)
                _buildTextField(label: 'Há quanto tempo fuma?', onChanged: (val) => _avaliacao.anaFumaTempo = val),
              SwitchListTile(
          ...
          GroupContainer(
            ...
            children: [
              _buildTextField(label: 'Horas médias de sono por noite', isNumber: true, onChanged: (val) => _avaliacao.anaHsSono = val),
              const SizedBox(height: 15),
              _buildFrequenciaSono(titulo: "a) Demorar mais de 30 minutos para dormir", valorAtual: _q5a, onChanged: (val) => setState(() => _q5a = val)),
              _buildFrequenciaSono(titulo: "b) Acordar no meio da noite ou manhã cedo", valorAtual: _q5b, onChanged: (val) => setState(() => _q5b = val)),
              _buildFrequenciaSono(titulo: "c) Levantar para ir ao banheiro", valorAtual: _q5c, onChanged: (val) => setState(() => _q5c = val)),
              _buildFrequenciaSono(titulo: "h) Ter sonhos ruins ou pesadelos", valorAtual: _q5h, onChanged: (val) => setState(() => _q5h = val)),
              _buildFrequenciaSono(titulo: "i) Sentir dores durante a noite", valorAtual: _q5i, onChanged: (val) => setState(() => _q5i = val)),

              const Divider(),
              _buildFrequenciaSono(titulo: "7) Tomou algum remédio para dormir?", valorAtual: _q7Frequencia, onChanged: (val) => setState(() => _q7Frequencia = val)),
              if (_q7Frequencia != null && _q7Frequencia! > 0)
                _buildTextField(label: 'Qual(is) remédios?', onChanged: (val) => _avaliacao.dadosSono['q7RemedioQuais'] = val),
      ...
      child: Column(
        children: [
          GroupContainer(
            ...
            children: [
              _buildTextField(label: 'Massa Corporal (kg)', isNumber: true, onChanged: (val) => _avaliacao.antPeso = double.tryParse(val)),
              const SizedBox(height: 12),
              _buildTextField(label: 'Estatura (cm)', isNumber: true, onChanged: (val) => _avaliacao.antAltura = double.tryParse(val)),
            ],
          ),
          GroupContainer(
            title: 'Perímetros',
            children: [
              _buildTextField(label: 'Cintura (cm)', isNumber: true, onChanged: (val) => _avaliacao.antPeriCintura = double.tryParse(val)),
              const SizedBox(height: 12),
              _buildTextField(label: 'Quadril (cm)', isNumber: true, onChanged: (val) => _avaliacao.antPeriQuadril = double.tryParse(val)),
            ],
          ),
        ],
      ),
  ...
  Widget _buildTestesFisicosDoresTab() {
    final regioesCorpo = ["A - Cabeça/Pescoço", "B - Ombros", "C - Braços", "D - Coluna Dorsal", "E - Coluna Lombar", "F - Quadril", "G - Joelhos", "H - Pés"];
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
            ...
            children: [
              ...
              if (_avaliacao.comDorehj) ...[
                const Padding(
                  ...
                  child: Text("Informe a intensidade da dor para cada região (1 a 10):", style: TextStyle( fontWeight: FontWeight.w600)),
                ),
                  ...
                  return Row(
                    children: [
                      ...
                      Expanded(
                        flex: 1,
                        child: Slider(
                          value: (_avaliacao.comDores[letraChave] ?? 0).toDouble(),
                          min: 0, max: 10, divisions: 10,
                          label: (_avaliacao.comDores[letraChave] ?? 0).toString(),
                          onChanged: (double esc) {
                            setState(() => _avaliacao.comDores[letraChave] = esc.toInt());
                          },
                        ),
                      ),
                    ],
          ...
          GroupContainer(
            ...
            children: [
              _buildPosturaDropdown(
                ...
                opces: ["Normal", "Inclinada para a Direita", "Inclinada para a Esquerda", "Rotada para a Direita", "Rotada para a Esquerda"],
                onChanged: (val) => setState(() => _avaliacao.posAnteriorCabeca = val), opcoes: [],
              ),
              _buildPosturaDropdown(
                label: '2 - Ombros',
                valorAtual: _avaliacao.posAnteriorOmbros,
                opces: ["Normal", "Elevado Direito", "Elevado Esquerdo", "Elevados Ambos", "Caídos"],
                onChanged: (val) => setState(() => _avaliacao.posAnteriorOmbros = val), opcoes: [],
              ),
              _buildPosturaDropdown(
                label: '10 - Joelhos',
                valorAtual: _avaliacao.posAnteriorJoelhos,
                opces: ["Normais", "Valgo Direito", "Valgo Esquerdo", "Valgo Ambos", "Varo Direito", "Varo Esquerdo", "Varo Ambos"],
                onChanged: (val) => setState(() => _avaliacao.posAnteriorJoelhos = val), opcoes: [],
              ),
            ],
          ),
          GroupContainer(
            ...
            children: [
              _buildPosturaDropdown(
                ...
                onChanged: (val) => setState(() => _avaliacao.posPerfilCabeca = val), opcoes: [],
              ),
              _buildPosturaDropdown(
                label: '13 - Ombros',
                valorAtual: _avaliacao.posPerfilOmbros,
                opces: ["Normais", "Anteriorizados", "Posteriorizados"],
                onChanged: (val) => setState(() => _avaliacao.posPerfilOmbros = val), opcoes: [],
              ),
            ],
          ),
          GroupContainer(
            ...
            children: [
              _buildPosturaDropdown(
                ...
                opces: ["Não apresenta", "Sim - Local: Lombar", "Sim - Local: Torácica", "Sim - Local: Cervical"],
                onChanged: (val) => setState(() => _avaliacao.posPosteriorEscoliose = val), opcoes: [],
              ),
              _buildPosturaDropdown(
                label: '21 - Teste de Flexão / Gibosidade',
                valorAtual: _avaliacao.posPosteriorGibosidade,
                opces: ["Não apresenta Gibosidade", "Apresenta Gibosidade - Local: Lombar", "Apresenta Gibosidade - Local: Torácica"],
                onChanged: (val) => setState(() => _avaliacao.posPosteriorGibosidade = val), opcoes: [],
              ),
            ],
          ),
        ],
  ...
  Widget _buildRelatorioTab() {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text("Avaliação do Aluno: ${widget.aluno.nome}", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          const SizedBox(height: 15),
          Expanded(
            child: TextFormField(
              maxLines: 10,
